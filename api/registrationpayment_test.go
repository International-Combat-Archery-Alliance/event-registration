...
func TestStripeRegistrationPaymentWebhookMiddleware(t *testing.T) {
	t.Run("successful payment confirmation webhook", func(t *testing.T) {
		...
		api := NewAPI(mockDB, noopLogger, LOCAL, newTestTokenValidator(), &mockCaptchaValidator{}, &mockEmailSender{}, &mockSubscriberManager{}, mockCheckout, func(context.Context) error { return nil })
